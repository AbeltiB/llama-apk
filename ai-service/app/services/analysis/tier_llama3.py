...
from app.services.analysis.intent_schemas import (
    ...
    ActionRecommendation, AppDomain, TechnicalRequirements,
...
class Llama3Tier(ClassificationTierBase):
    ...
    def __init__(self, llama_config: Dict[str, Any]):
        ...
        super().__init__(
            tier=ClassificationTier.LLAMA3,
            retry_config=config.TIERS["llama3"].retry_config if hasattr(config, 'TIERS') else None
        )
    ...
    async def _classify_internal(
        ...
        try:
            ...
            deep_entities = self.entity_extractor.extract_heuristic(request.prompt)
            ...
            result.total_cost_usd = (tokens_used / 1000) * 0.001
    ...
    def _build_system_prompt(self) -> str:
        """Build comprehensive system prompt for Llama3"""
...
"""
    ...
    def _parse_llama3_response(
        ...
        intent_type = IntentType.from_string(parsed.get('intent_type', 'other'))
        
        # Parse domain
        domain_str = parsed.get('domain', 'custom')
        try:
        ...
        tech_data = parsed.get('technical_requirements', {})
