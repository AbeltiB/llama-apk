...
class IntentAnalysisStage(PipelineStage):
    ...
    def __init__(self):
        ...
        #Use the global intent_analyzer instance that is configured to use Llama3
        from app.services.analysis.intent_analyzer import intent_analyzer
    ...
    async def execute(self, request: AIRequest, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        #Add exisitng project info if available to intent for better logging and downstream use
        if context.get('existing_project'):
            analysis_context['existing_project'] = context['existing_project']
            analysis_context['project_confidence'] = context.get('project_confidence', 0.5)
        
        #Add conversation history
        if context.get('conversation_history'):
            analysis_context['conversation_history'] = context['conversation_history']
            
        # Analyze intent
        intent = await self.analyzer.analyze(
            prompt=request.prompt,
            user_id=request.user_id,
 ...
 # If clarification needed, add to context
        if hasattr(intent, 'action_recommendation') and intent.action_recommendation == ActionRecommendation.CLARIFY:
            context['needs_clarification'] = True
            if hasattr(intent, 'suggested_clarifications'):
                context['clarification_questions'] = intent.suggested_clarifications
        
        # If unsafe, stop pipeline
        if hasattr(intent, 'safety_status') and intent.safety_status == SafetyStatus.UNSAFE:
            logger.error(
                "pipeline.intent_analysis.unsafe",
                extra={"reasoning": getattr(intent, 'safety_reasoning', 'No reason provided')}
            )
            raise ValueError(f"Unsafe request: {getattr(intent, 'safety_reasoning', 'Request flagged as unsafe')}")
...
class ContextBuildingStage(PipelineStage):
    ...
    async def execute(self, request: AIRequest, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract intent info for logging
        if isinstance(intent, dict):
            intent_domain = intent.get('domain', 'unknown')
            if hasattr(intent_domain, 'value'):
                intent_domain = intent_domain.value
            intent_complexity = intent.get('complexity', 'unknown')
            if hasattr(intent_complexity, 'value'):
                intent_complexity = intent_complexity.value
        else:
        ...
        logger.info(
            ...
            extra={
                ...
                "project_confidence": enriched_context.project_confidence,
                "history_messages": len(enriched_context.conversation_history),
                "user_expertise": enriched_context.user_expertise
            }
        )
...
class ArchitectureGenerationStage(PipelineStage):
    ...
    async def execute(self, request: AIRequest, context: Dict[str, Any]) -> Dict[str, Any]:
        """Generate architecture USING ENRICHED CONTEXT"""
        ...
        try:
            ...
            logger.info(
                ...
                extra={
                    "app_name": architecture.app_name if hasattr(architecture, 'app_name') else 'unknown app name',
                    "app_type": architecture.app_type if hasattr(architecture, 'app_type') else 'unknown app type',
                    "screens": len(architecture.screens) if hasattr(architecture, 'screens') else 0,
                    "components": sum(len(screen.components) for screen in architecture.screens) if hasattr(architecture, 'screens') else 0,
                    "generation_method": metadata.get('generation_method', 'unknown')
                }
            )
            ...
            return {
                ...
                "component_count": sum(len(screen.components) for screen in architecture.screens),
                "metadata": metadata
            }
    ...
    async def send_progress(
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    logger.error(f"Failed to send progress after 3 attempts: {e}")
